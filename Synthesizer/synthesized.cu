...
__global__ void diffusion_kernel(float* in,
  ...
  for (int k = 0; k < nz; ++k) {
    ...
    out[c] = 
        cc * in[c] 
      + cw * ((!(0==threadIdx.x)||(i==threadIdx.x)) ? sb[!(0==threadIdx.x) ? (csb - 1) : csb] : in[w]) 
      + ce * ((!((i - 1)==blockDim.y)&&!((blockDim.x - 1)==i)) ? sb[!((threadIdx.x - 1)==1) ? (1 + csb) : csb] : in[e]) 
      + cs * ((((csb - 1)<j)||!(blockDim.x==threadIdx.y)) ? sb[!(blockDim.x==threadIdx.y) ? (blockDim.x + csb) : csb] : in[s])
      + cn * (((j==threadIdx.y)||!(csb==threadIdx.x)) ? sb[!(csb==threadIdx.x) ? (csb - blockDim.x) : threadIdx.x] : in[n]) 
      + cb * in[b] 
      + ct * in[t];   
    c += xy;
  }
