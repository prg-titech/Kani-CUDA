...
__global__ void jacobi(float *a0, float *a1, float *a2, float *a3, float *b0, float *b1, float *b2, float *c0, float *c1, float *c2, float *p, float *wrk1, float *wrk2, float *bnd, int nn, int imax, int jmax, int kmax, float omega, float *gosa){
	...
	for(n=0;n<nn;++n){
		...
		for(i=1 ; i<imax-1 ; ++i){
			...
			+ a1[i*jmax*kmax+j*kmax+k] * (((threadIdx.y + 1)!=blockDim.y) ? sb[ csb + blockDim.x ] : p[i*jmax*kmax+(j+1)*kmax+k])
			...
			+ b1[i*jmax*kmax+j*kmax+k] *(
				((((threadIdx.y + 1)!=blockDim.y)&&(blockDim.y!=threadIdx.x)) ? sb[ csb + blockDim.x + 1 ] : p[i*jmax*kmax+(j+1)*kmax+(k+1)])
				...
				+ ((((threadIdx.y + 1)!=blockDim.y)&&(threadIdx.x!=0)) ? sb[ csb + blockDim.y ] : p[i*jmax*kmax+(j+1)*kmax+(k-1)]))
