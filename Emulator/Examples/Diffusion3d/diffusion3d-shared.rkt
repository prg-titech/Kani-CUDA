...
(define (diffusion-kernel-shared in
  ...
  (syncthreads)
  (for- [: (</LS k (-/LS k-end 1)) : (++/LS k)]
        (= t1 t2)
        (= t2 t3)
        (syncthreads)
        (= [sb c1] t2)
        (= t3 [in (+/LS c xy)])
        (:= float t (+/LS (*/LS cc t2) (*/LS cb t1) (*/LS ct t3)))
        (syncthreads)
        (+=/LS t (*/LS cw (?: bw [in (-/LS c 1)] [sb w])))
        (+=/LS t (*/LS ce (?: be [in (+/LS c 1)] [sb e])))
        (+=/LS t (*/LS cs (?: bs [in (+/LS c nx)] [sb s])))
        (+=/LS t (*/LS cn (?: bn [in (-/LS c nx)] [sb n])))
        (= [out c] t)
        (+=/LS c xy)
        (syncthreads))
  (= t1 t2)
