...
(define vars (open-output-file "vars" #:exists 'truncate))
(fprintf vars "tid bid id smid threadIdx.x threadIdx.y blockDim.x blockDim.y csb BLOCKSIZE a b c d e f g h\n")
             ...
             (+/LS
              (*/LS
               ...
               (profiling-access3
                ...
                (thread-idx 0) (thread-idx 1) (block-dim 0) (block-dim 1) csb BLOCKSIZE i j k jmax kmax imax c xy))
              ;               (profiling-access
              ;                out-file
              ;                p
              ;                (+/LS
  ...
  (for-
   ...
   (for-
    ...
    (for-
     ...
     (array-set-host! p (+ (+ (* (* i mjmax) mkmax) (* j mkmax)) k) (r));(+ (+ (* (* i mjmax) mkmax) (* j mkmax)) k));(/ (* k k) (* (- mkmax 1) (- mkmax 1))))
